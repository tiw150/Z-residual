...
simul_ph_nonlinear <- function(n_clusters,n_individuals,lambda, alpha, 
                               mean.censor,beta_a,beta_b,beta1,t0,corr)
{
  ...
  x1 = round(covar[,1],1)
  x2 = pmax(abs(round(covar[,2],1)),0.01)
...
simul_nonph_linear <- function(n_clusters,n_individuals,lambda, alpha, 
                               mean.censor,beta_a,beta_b,beta1,t0,corr)
{
  # Make covariates 
  n<-n_individuals * n_clusters
